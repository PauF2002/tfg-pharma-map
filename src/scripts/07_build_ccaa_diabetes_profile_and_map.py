...
DIAB = BASE / "data" / "processed" / "ccaa_diabetes.csv"

OUT_PROFILE = BASE / "data" / "processed" / "ccaa_diabetes_profile_latest.csv"
OUT_SCORE = BASE / "data" / "processed" / "ccaa_diabetes_opportunity_score.csv"
OUT_MAP = BASE / "outputs" / "maps" / "ccaa_map_diabetes.html"
...
d["ccaa_key"] = d["CCAA"].map(norm_txt)
...
diab = d[["ccaa_key", "diabetes_val"]].copy()

profile = h.merge(market12, on="ccaa_key", how="left").merge(diab, on="ccaa_key", how="left")

profile["beds_per_100k"] = pd.to_numeric(profile["beds_per_100k"], errors="coerce").fillna(0)
profile["market_12m_avg_eur_per_capita"] = pd.to_numeric(
    profile["market_12m_avg_eur_per_capita"], errors="coerce"
).fillna(0)
profile["diabetes_val"] = pd.to_numeric(profile["diabetes_val"], errors="coerce").fillna(0)

profile["beds_n"] = minmax(profile["beds_per_100k"])
profile["market_n"] = minmax(profile["market_12m_avg_eur_per_capita"])
profile["diabetes_n"] = minmax(profile["diabetes_val"])

profile["opportunity_score"] = (
    100 * (0.45 * profile["market_n"] + 0.35 * profile["diabetes_n"] + 0.20 * profile["beds_n"])
).round(2)
...
geo_keys = sorted(
    {feat["properties"]["ccaa_key"] for feat in gj["features"] if feat["properties"]["ccaa_key"]}
)
aliases = {
...
fig.update_layout(
    title="Opportunity score for diabetes by CCAA",
